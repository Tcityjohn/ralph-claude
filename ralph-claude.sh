...
#
# This is Ralph adapted for Claude Code (Anthropic's CLI).
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  PROMPT=$(cat "$SCRIPT_DIR/prompt-claude.md")

  # Run Claude Code with the ralph prompt
  # --dangerously-skip-permissions: allows autonomous operation
  # --print: outputs the response (non-interactive mode)
  # We use --print and pipe to capture output while still showing it
